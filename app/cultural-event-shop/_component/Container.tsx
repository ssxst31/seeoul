    ...
    <>
      ...
      <ul className="grid grid-cols-1 gap-y-8 gap-x-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8 -md:gap-y-4 -md:gap-x-4">
        {data?.items?.map((el: any) => (
          <li className="relative w-full overflow-hidden cursor-pointer dark:shadow-gray-900">
            <a href={el.link} key={el.productId} className="inline-block w-full">
              ...
              <div className="p-2">
                <strong
                  ...
                  className="block w-full overflow-hidden text-base font-bold text-black text-ellipsis webkit-box webkit-line-clamp-2 webkit-box-vertical dark:text-white"
                />
